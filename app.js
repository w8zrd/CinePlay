...
const renderRecent = () => {
  ...
  g.innerHTML=arr.map(item=>{
    const img=item.poster?`<img src="${esc(item.poster)}" alt="${esc(item.title)}" loading="lazy"/>`:`<div class="mini-noposter">🎬</div>`;
...
const buildFilterChips = () => {
  ...
  $('rating-chips').innerHTML=[null,9,8,7,6].map((r,i)=>
    `<button class="chip${FP.rating===r?' active':''}" data-rating="${r}">${['All','9+ ★','8+ ★','7+ ★','6+ ★'][i]}</button>`
  ).join('');
...
const showHeroSlide = (idx) => {
  ...
  $('hero-watch-btn').onclick=async()=>{
    const t=$('hero-watch-btn').textContent;
    ...
    $('hero-watch-btn').textContent=t;
  };
  $('hero-more-btn').onclick=()=>openModal(m.id,'movie');
};

const heroNext=()=>{showHeroSlide(heroIdx+1);resetHT();};
const heroPrev=()=>{showHeroSlide(heroIdx-1);resetHT();};
const resetHT=()=>{clearInterval(heroTimer);heroTimer=setInterval(heroNext,6000);};
...
const initHero=async()=>{
  try{
    ...
    $('hero-dots').addEventListener('click',e=>{
      ...
      if(b){showHeroSlide(parseInt(b.dataset.dot));resetHT();}
    });
    ...
    hero.addEventListener('touchend',e=>{
      ...
      const dy=Math.abs(e.changedTouches[0].clientY-tyS);
      const dt=Date.now()-ttS;
      if(Math.abs(dx)>=80&&dy<40&&Math.abs(dx)>dy*2&&dt>80&&dt<700){dx<0?heroNext():heroPrev();}
    },{passive:true});
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  // Filter sheet
  $('filter-btn').addEventListener('click',e=>{e.stopPropagation();openFilterSheet();});
  $('filter-overlay').addEventListener('click',closeFilterSheet);
  ...
  ['genre-chips','year-chips','rating-chips'].forEach(id=>{
    $(id).addEventListener('click',e=>{
      ...
      if(!btn)return;
      if('genre' in btn.dataset){const gid=parseInt(btn.dataset.genre);FP.genreId=FP.genreId===gid?null:gid;}
  ...
  $('filter-apply').addEventListener('click',e=>{
    ...
    closeFilterSheet();buildFilterChips();fetchContent();
  });
