...
const addRecent = (item) => {
  let a = getRecent().filter(i=>i.id!==item.id);
  a.unshift(item); a=a.slice(0,20);
  try{localStorage.setItem(RKEY,JSON.stringify(a));}catch{}
  renderRecent();
};
const renderRecent = () => {
  ...
  const g=$('recent-grid');
  if (!g) return;
  g.innerHTML=arr.map(item=>{
    const img=item.poster?`<img src="${esc(item.poster)}" alt="${esc(item.title)}" loading="lazy"/>`:`<div class="mini-noposter">No img</div>`;
    return `<div class="mini-card" data-id="${item.id}" data-type="${item.type}" tabindex="0" role="button">${img}<div class="mini-title">${esc(item.title)}</div></div>`;
  }).join('');
};
...
const closeFilter = () => { $('filter-panel').style.display='none'; };

// ══ SECTION TITLE ════════════════════════════
const updateTitle = () => {
  const mode=S.mode==='movie'?'Movies':'TV Shows';
  const map={popular:'Popular',trending:'Trending',top_rated:'Top Rated',upcoming:S.mode==='movie'?'Upcoming':'On The Air',search:'Search Results'};
  $('section-title').textContent=(map[S.tab]||'')+' '+mode;
};
...
const openModal = async (id, type) => {
  ...
  $('modal-backdrop').src='';
  $('modal-poster').src=''; $('modal-poster').style.display='none';
  ...
  try {
    ...
    if (cast.length){
      ...
      $('modal-cast').innerHTML=cast.map(c=>{
        const av=c.profile_path?`<img class="cast-av" src="${TMDB_IMG}/w45${c.profile_path}" alt="${esc(c.name)}" loading="lazy"/>`:`<div class="cast-av-ph">👤</div>`;
...
// ══ HERO — loads in parallel with grid ═══════
let heroMovies=[], heroIdx=0, heroTimer=null;
let txStart=0, tyStart=0, ttStart=0;
...
const showHeroSlide = (idx) => {
  ...
  // update slide position instantly
  ...
  $('hero-watch-btn').onclick=async ()=>{
    const orig=$('hero-watch-btn').textContent;
    $('hero-watch-btn').textContent='Loading…';
    try{const d=await tmdb(`/movie/${m.id}`);if(d.imdb_id)window.open(playUrl(d.imdb_id),'_blank');}catch{}
    $('hero-watch-btn').textContent=orig;
  };
  $('hero-more-btn').onclick=()=>openModal(m.id,'movie');
};

const heroNext=()=>{showHeroSlide(heroIdx+1);resetHeroTimer();};
const heroPrev=()=>{showHeroSlide(heroIdx-1);resetHeroTimer();};
const resetHeroTimer=()=>{clearInterval(heroTimer);heroTimer=setInterval(heroNext,6000);};

const initHero=async()=>{
  try{
    // fetch trending in parallel — don't block grid
    ...
    track.innerHTML=heroMovies.map(m=>`<div class="hero-slide" style="flex:0 0 ${100/heroMovies.length}%;background-image:url('${backdropUrl(m.backdrop_path)}')"></div>`).join('');
    ...
    $('hero-dots').addEventListener('click',e=>{
      ...
      if(b){showHeroSlide(parseInt(b.dataset.dot));resetHeroTimer();}
    });

    // arrows
    $('hero-prev').addEventListener('click',e=>{e.stopPropagation();heroPrev();});
    $('hero-next').addEventListener('click',e=>{e.stopPropagation();heroNext();});

    // touch swipe — strict: 80px+, horizontal > 2x vertical, 80-700ms
    const hero=$('hero');
    hero.addEventListener('touchstart',e=>{
      txStart=e.touches[0].clientX;tyStart=e.touches[0].clientY;ttStart=Date.now();
    },{passive:true});
    hero.addEventListener('touchend',e=>{
      const dx=e.changedTouches[0].clientX-txStart;
      const dy=Math.abs(e.changedTouches[0].clientY-tyStart);
      const dt=Date.now()-ttStart;
      if(Math.abs(dx)>=80&&dy<40&&Math.abs(dx)>dy*2&&dt>80&&dt<700){
...
const switchMode = (mode) => {
  ...
  if(up) up.textContent=mode==='tv'?'On The Air':'Upcoming';
  closeFilter();
  buildFilterChips();
  fetchContent();
};
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  $('filter-btn').addEventListener('click',e=>{
    e.stopPropagation();
    $('filter-panel').style.display==='none'?openFilter():closeFilter();
  });

  // Click outside to close filter
  document.addEventListener('click',e=>{
    if($('filter-panel').style.display==='none')return;
    if($('filter-panel').contains(e.target))return;
    if($('filter-btn').contains(e.target))return;
    closeFilter();
  });

  // Chip clicks — each group stops propagation
  ...
  $('filter-apply').addEventListener('click',e=>{
    ...
    closeFilter();buildFilterChips();fetchContent();
  });
  ...
  // ── BOOT: fire grid + hero IN PARALLEL ──
  $('filter-panel').style.display='none';
  buildFilterChips();
  renderRecent();
  // fetch grid and hero at same time
  fetchContent();
  initHero();
});
