...
const initHero = async () => {
  try {
    ...
    // arrows - stop propagation so they don't trigger other events
    ...
    // TOUCH — strict swipe: must be 80px+ horizontal, less than 40px vertical, takes >100ms
    const hero=$('hero');
    let touchTime=0;
    ...
    hero.addEventListener('touchmove', e=>{
      ...
      // only count as dragging if clearly horizontal
      if (dx>20&&dx>dy*2) isDragging=true;
    },{passive:true});
    hero.addEventListener('touchend', e=>{
      ...
      // require: 80px+ swipe, <40px vertical drift, between 80ms and 800ms
      if (Math.abs(dx)>=80&&dy<40&&dt>80&&dt<800) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const openFilter = () => {
    ...
    $('filter-btn').classList.add('active-open');
  };
  const closeFilter = () => {
    $('filter-panel').style.display='none';
    $('filter-btn').classList.remove('active-open');
  };
  ...
  // Boot — ensure panels start hidden
  $('filter-panel').style.display='none';
